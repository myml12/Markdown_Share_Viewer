  ...
  try {
    ...
    if (gistInput.includes('gist.github.com')) {
      ...
      if (match) {
        ...
        // デフォルトで最初のファイルを取得（ファイル名が分からない場合は空文字列）
        return `https://gist.githubusercontent.com/${username}/${gistId}/raw/`;
      }
    }

    // Gist IDのみの場合（ユーザー名が必要なので、raw URLの形式を推測）
    // 実際には、Gist APIを使ってファイル一覧を取得する必要がある
    // 簡易版として、よくあるパターンを試す
    if (/^[a-f0-9]+$/i.test(gistInput)) {
      // Gist IDのみの場合は、API経由でファイル一覧を取得する必要がある
      // ここでは簡易的に、よくあるファイル名を試す
